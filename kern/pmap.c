...
boot_alloc(uint32_t n)
{
  ...
  if (n > 0) {
    ...
    // ? return a kernel virtual address, which?
  } else if (n < 0) {
...
mem_init(void)
{
  ...
  cprintf("create initial page directory.\n");
...
page_init(void)
{
  ...
  int mpentry_startp = MPENTRY_PADDR / PGSIZE;
  int mpentry_endp = ROUNDUP(MPENTRY_PADDR + mpentry_end - mpentry_start - 1,
      PGSIZE) / PGSIZE;
	for (i = 1; i < npages_basemem; i++) {
    if (i >= mpentry_startp && i < mpentry_endp) {
      pages[i].pp_ref = 1;
  ...
  // IOPHYSMEM == 0x0A0000
  // What if npages_basemem * PGSIZE not conform with IOPHYSMEM?
  // TODO: to decide which part of extended memory has been allocated
  // PGSIZE + npages * sizeof(struct PageInfo)
  for ( ; i < EXTPHYSMEM / PGSIZE ; i++) {
    pages[i].pp_ref = 1;
    pages[i].pp_link = NULL;
  }
  kern_used = ((uint32_t)boot_alloc(0) - KERNBASE) / PGSIZE;
  cprintf("page_init: kern_used: 0x%08x\n", kern_used);
  for (i = EXTPHYSMEM / PGSIZE ; i < kern_used ; i++) {
    pages[i].pp_ref = 1;
    pages[i].pp_link = NULL;
  }
  for ( ; i < npages ; i++) {
...
page_free(struct PageInfo *pp)
{
  ...
  if (pp->pp_ref != 0) {
    panic("page_free: pp->pp_ref != 0, pp = %p", pp);
  }
  if (pp->pp_link != NULL) {
    panic("page_free: pp->pp_link != NULL, pp = %p", pp);
  }
  ...
  if (pgdir[pdx] == 0) {
    if (create) {
      ...
      if (newpg == NULL) {
        ...
        newpg->pp_ref = 1;
        pgdir[pdx] = PTE_ADDR(page2pa(newpg)) | PTE_P | PTE_W | PTE_U;
      }
    } else {
      return NULL;
    }
  }
  pte_addr = (pte_t*)KADDR(PTE_ADDR(pgdir[pdx])) + ptx;
  return pte_addr;
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  for (i = 0 ; i < page_num ; ++i) {
    pte_addr = pgdir_walk(pgdir, (int*)(va+i*PGSIZE), true);
    *pte_addr = PTE_ADDR(pa + i*PGSIZE) | perm | PTE_P;
  }
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
  ...
  if (PTE_ADDR(*pte_addr) != PTE_ADDR(newpte)) { // already a page mapped at 'va'
...
mmio_map_region(physaddr_t pa, size_t size)
{
  ...
  boot_map_region(kern_pgdir, reserve_base, size, pa, PTE_P|PTE_W|PTE_PCD|PTE_PWT);
...
check_page_installed_pgdir(void)
{
  ...
  // test for self-mapping
  cprintf("%08x\n", *(int*)(PGADDR(PDX(UVPT), PDX(0xf0000000), PTX(0xf0000000) << 2)));
  assert(PTE_ADDR(*(int*)(PGADDR(PDX(UVPT), PDX(0xf0000000), PTX(0xf0000000) << 2))) == 0x00000000);
